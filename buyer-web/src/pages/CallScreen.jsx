...
const css = `
  ...
  .ar-toggle { padding:0 18px; height:44px; border:1px solid rgba(255,255,255,0.12); background:rgba(255,255,255,0.05); color:rgba(255,255,255,0.6); font-family:var(--font-body); font-size:11px; letter-spacing:1.5px; text-transform:uppercase; cursor:pointer; transition:all var(--transition); }
  .ar-toggle:hover { border-color:rgba(255,255,255,0.3); color:var(--white); }
  .ar-toggle.on { background:rgba(255,255,255,0.15); border-color:rgba(255,255,255,0.4); color:var(--white); }
...
export default function CallScreen() {
  ...
  useEffect(() => {
    ...
    socket.on('call_rejected',({callSessionId:cid})=>{ if(cid===callSessionId) setCallStatus('rejected'); });
    socket.on('call_message',msg=>setMessages(p=>[...p,msg]));
    socket.on('call_ended',({callSessionId:cid})=>{ if(cid===callSessionId) endCall(false); });
    return ()=>{ socket.off('call_accepted'); socket.off('call_rejected'); socket.off('call_message'); socket.off('call_ended'); };
  },[callSessionId]);

  const joinAgora = useCallback(async (ch) => {
    try {
      const tr = await api.post('/agora/call-channel',{callSessionId});
      await client.join(import.meta.env.VITE_AGORA_APP_ID||'YOUR_AGORA_APP_ID', ch||tr.data.channelName, tr.data.token, null);
      localTracks.audio = await AgoraRTC.createMicrophoneAudioTrack();
      localTracks.video = await AgoraRTC.createCameraVideoTrack();
      localTracks.video.play('cs-local-container');
      await client.publish([localTracks.audio,localTracks.video]);
      client.on('user-published', async (u,t)=>{ await client.subscribe(u,t); if(t==='video') u.videoTrack?.play('cs-remote-container'); if(t==='audio') u.audioTrack?.play(); });
  ...
  const endCall = useCallback((emit=true)=>{
    ...
    setCallStatus('ended');
    if (emit) { socket.emit('call_ended',{callSessionId}); api.patch(`/calls/${callId}/end`).catch(()=>{}); }
    localTracks.audio?.close(); localTracks.video?.close(); client.leave().catch(()=>{});
  ...
  const toggleAR = ()=>{ const n=!arActive; setArActive(n); socket.emit(n?'ar_started':'ar_stopped',{callSessionId}); };
      ...
      <div className="cs-root">
        ...
        <div className="cs-body">
          <div className="cs-video-col">
            ...
            {callStatus==='connecting'&&<div className="cs-waiting"><div className="cs-waiting-icon">📞</div><h3>Calling seller…</h3><p>Waiting for them to pick up</p></div>}
            {callStatus==='rejected'&&<div className="cs-ended"><div style={{fontSize:40}}>📵</div><h3 style={{fontFamily:'var(--font-display)',fontSize:24,fontWeight:300,color:'rgba(255,255,255,0.4)'}}>Call rejected</h3><button style={{background:'none',border:'1px solid rgba(255,255,255,0.2)',color:' rgba(255,255,255,0.5)',padding:'10px 24px',cursor:'pointer',fontFamily:'var(--font-body)',fontSize:11,letterSpacing:2,textTransform:'uppercase'}} onClick={()=>navigate('/')}>Go Back</button></div>}
            {callStatus==='ended'&&<div className="cs-ended"><div style={{fontSize:40}}>👋</div><h3 style={{fontFamily:'var(--font-display)',fontSize:24,fontWeight:300,color:'rgba(255,255,255,0.4)'}}>Call ended · {fmt(elapsed)}</h3><button style={{background:'var(--white)',border:'none',color:'var(--black)',padding:'12px 28px',cursor:'pointer',fontFamily:'var(--font-body)',fontSize:11,letterSpacing:2,textTransform:'uppercase'}} onClick={()=>navigate('/')}>Back to Home</button></div>}
            {(callStatus==='active'||callStatus==='connecting')&&<div className="cs-local-pip"><div id="cs-local-container"/><div className="pip-label">You</div></div>}
            {callStatus==='active'&&(
              <div className="cs-controls">
                <button className={`ctrl-btn ${muted?'active':''}`} onClick={toggleMute}>{muted?'🔇':'🎙'}</button>
                <button className={`ctrl-btn ${videoOff?'active':''}`} onClick={toggleVideo}>{videoOff?'📷':'📹'}</button>
                <button className={`ar-toggle ${arActive?'on':''}`} onClick={toggleAR}>{arActive?'AR On':'Try AR'}</button>
          ...
          <div className="cs-sidebar">
              ...
              <div className="cs-products-panel">
                ...
                {products.slice(0,5).map(p=>(
                  <div key={p.id} className="cs-product-row" onClick={()=>navigate(`/product/${p.id}`,{state:{product:p}})}>
                    <div className="cs-prod-thumb">{p.image_urls?.[0]?<img src={p.image_urls[0]} alt={p.name}/>:'👗'}</div>
            ...
            <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
              {callStatus==='active' ? <ChatBox messages={messages} onSend={handleSend} currentUserId={user.id} placeholder="Message seller…"/> : <div className="cs-ended-note">{callStatus==='connecting'?'Chat will appear once connected.':'Call has ended.'}</div>}
            </div>
          </div>
        </div>
      </div>
